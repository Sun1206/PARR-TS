...
def train_patchtst(x, y, train_idx, val_idx, args, seed):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        history.append(
            {"epoch": epoch, "train_loss": float(np.mean(losses)), "val_loss": val_loss}
        )
        print(
            f"epoch {epoch:02d} train={history[-1]['train_loss']:.6f} val={val_loss:.6f}",
            flush=True,
        )
...
def evaluate(args):
    ...
    x_raw, y_raw = base.make_windows(arr, 96, 96, args.max_windows, seed=args.seed)
...
def main():
    ...
    parser.add_argument("--outdir", default="local_experiments/results_traffic_patchtst_small")
    ...
    df.to_csv(outdir / "traffic_patchtst_small_scores.csv", index=False)
    summary = df.sort_values(["top25_reduction", "risk_coverage_auc"], ascending=False)
    summary.to_csv(outdir / "traffic_patchtst_small_summary.csv", index=False)
