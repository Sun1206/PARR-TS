...
while true; do
  ...
  if (( ok_count >= CHECKS )); then
    echo "$(date) probing torch import with timeout=${TORCH_TIMEOUT}s"
    if timeout "${TORCH_TIMEOUT}" python - <<'PY'
import torch
print({"cuda_available": torch.cuda.is_available(), "device_count": torch.cuda.device_count()})
PY
    then
      echo "$(date) torch ready; starting ${SCRIPT}"
      bash "${SCRIPT}" > "${LOG}" 2>&1
      echo "$(date) finished ${SCRIPT}"
      break
    fi
    echo "$(date) torch probe failed or timed out; waiting"
    ok_count=0
  fi
  sleep "${SLEEP_SECONDS}"
done
